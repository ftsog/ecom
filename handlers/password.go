...
func (app *Handler) UpdatePassword(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		checker, _ := app.Db.CheckSession(w, r)
		if !(*checker) {
		...
		usr, err := app.Db.GetUserFromSession(w, r)
		if err != nil {
		...
		if ud.Password != cPassword {
			...
			JsonResponseWriter(JsonResponse{
				...
				Data: responseError{
					...
					Detail:  "input password does not match current password",
				},
				statusCode: http.StatusBadRequest,
			})
...
func (app *Handler) ResetPassword(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		...
		err = utils.NewMail(*eml, subject, utils.ForgetMessage, ud.Username, token)
		if err != nil {
			app.Logger.ErrorLog.Println(err.Error())
			JsonResponseWriter(JsonResponse{
				ResponseWriter: w,
