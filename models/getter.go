...
func (dbExec *Model) GetByUsername(username string) (*userDetails, error) {
	ud := userDetails{}
	pstmt := `Select user_id, username, email, password, first_name, last_name, phone, address FROM users WHERE username=$1`
	stmt, err := dbExec.DB.Prepare(pstmt)
	if err != nil {
		return nil, err
	}

	err = stmt.QueryRow(username).Scan(&ud.UserId, &ud.Username, &ud.Email, &ud.Password, &ud.FirstName, &ud.LastName, &ud.Phone, &ud.Address)
	if err != nil {
		return nil, err
	}
